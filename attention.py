...
class BatchNorm(BatchNormalization):
    def call(self, inputs, training=None):
        return super(self.__class__, self).call(inputs, training=True)  # TODO: training?? should be set by default / obsolete
...
# TODO: verify functionality of new implementation ?
# DONE
class Repeat(Layer):
...
def SpatialAttention(inputs, name):
    k = 9
    H, W, C = list(map(int, inputs.get_shape()[1:]))
